...
void handleError (HWND hwndDlg, int code, const char* srcPos)
{
	...
	case ERR_INVALID_OFFSET: //Joakim
		MessageBoxW (hwndDlg, GetString ("INVALID_OFFSET"), lpszTitle, ICON_HAND);
...
retry:
	...
	mount.SecretOffset = SecretOffset;  //Joakim
	if (!bDevice)
	{
		// put default values
