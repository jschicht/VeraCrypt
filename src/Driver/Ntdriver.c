...
//Joakim
NTSTATUS TCOpenFsVolume (PEXTENSION Extension, PHANDLE volumeHandle, PFILE_OBJECT * fileObject, __int64 SecretOffset)
{
	NTSTATUS ntStatus;
	OBJECT_ATTRIBUTES objectAttributes;
	UNICODE_STRING fullFileName;
	...
	else
	{
		...
		else
		{
			if (mount->nReturnCode == 0)
			{
				...
				//Joakim
				if (NT_SUCCESS (TCOpenFsVolume (NewExtension, &volumeHandle, &volumeFileObject, mount->SecretOffset)))
				{
					__try
					{
						ULONG fsStatus;
...
NTSTATUS UnmountDevice (UNMOUNT_STRUCT *unmountRequest, PDEVICE_OBJECT deviceObject, BOOL ignoreOpenFiles)
{
	...
	//Joakim
	ntStatus = TCOpenFsVolume (extension, &volumeHandle, &volumeFileObject, 0);
