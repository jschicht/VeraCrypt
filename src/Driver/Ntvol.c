	       ...
	       BOOL bRawDevice)
{
	...
	//Joakim
	if (mount->SecretOffset % 512) {
		mount->nReturnCode = ERR_INVALID_OFFSET;
		goto error;
	}
		...
		volumeType++)
	{
			...
			|| (mount->bPartitionInInactiveSysEncScope && volumeType == TC_VOLUME_TYPE_HIDDEN))
		{
			...
			case TC_VOLUME_TYPE_NORMAL:
				headerOffset.QuadPart = mount->UseBackupHeader ? lDiskLength.QuadPart - TC_VOLUME_HEADER_GROUP_SIZE : mount->SecretOffset + TC_VOLUME_HEADER_OFFSET; //Joakim
				break;
			...
			case TC_VOLUME_TYPE_HIDDEN:
				...
				headerOffset.QuadPart = mount->UseBackupHeader ? lDiskLength.QuadPart - TC_HIDDEN_VOLUME_HEADER_OFFSET : mount->SecretOffset + TC_HIDDEN_VOLUME_HEADER_OFFSET; //Joakim
				break;
			}
		...
		if (mount->nReturnCode == 0 || mount->nReturnCode == ERR_CIPHER_INIT_WEAK_KEY)
		{
			...
			case TC_VOLUME_TYPE_NORMAL:
				...
				if (mount->bPartitionInInactiveSysEncScope)
				{
					Extension->cryptoInfo->volDataAreaOffset = mount->SecretOffset; //Joakim
					Extension->DiskLength = lDiskLength.QuadPart;
				...
				{
					Extension->cryptoInfo->volDataAreaOffset = mount->SecretOffset + TC_VOLUME_HEADER_SIZE_LEGACY; //Joakim
					Extension->DiskLength = lDiskLength.QuadPart - TC_VOLUME_HEADER_SIZE_LEGACY;
				}
				else
				{
					Extension->cryptoInfo->volDataAreaOffset = mount->SecretOffset + Extension->cryptoInfo->EncryptedAreaStart.Value; //Joakim
			...
			case TC_VOLUME_TYPE_HIDDEN:
				...
				Extension->cryptoInfo->hiddenVolumeOffset = mount->SecretOffset + cryptoInfoPtr->EncryptedAreaStart.Value; //Joakim
