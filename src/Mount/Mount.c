...
//Joakim:
__int64 ContainerOffset = 0;			/* Optional offset of container */
__int64 CmdContainerOffset = 0;			/* Offset of container passed from command line*/
wchar_t TmpContainerOffset[32];
...
static BOOL Mount (HWND hwndDlg, int nDosDriveNo, wchar_t *szFileName, int pim, int pkcs5, int trueCryptMode)
{
	...
	if (!bUseCmdVolumePassword)
	{
		...
		if (EffectiveVolumeTrueCryptMode)
			//Joakim
			mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, NULL, EffectiveVolumePkcs5, 0, TRUE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
		else
			//Joakim
			mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, NULL, EffectiveVolumePkcs5, EffectiveVolumePim, FALSE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
		...
		if (!mounted && KeyFilesEnable && FirstKeyFile && bEffectiveTryEmptyPasswordWhenKeyfileUsed)
		{
			...
			if (EffectiveVolumeTrueCryptMode)
				//Joakim
				mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &emptyPassword, EffectiveVolumePkcs5, 0, TRUE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
			else
				//Joakim
				mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &emptyPassword, EffectiveVolumePkcs5, EffectiveVolumePim, FALSE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
	...
	if (!mounted && bEffectiveCacheDuringMultipleMount && MultipleMountOperationInProgress && VolumePassword.Length != 0)
	{
		...
		if (EffectiveVolumeTrueCryptMode)
			//Joakim
			mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &VolumePassword, EffectiveVolumePkcs5, 0, TRUE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
		else // if no PIM specified for favorite, we use also the PIM of the previous volume alongside its password.
			//Joakim
			mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &VolumePassword, EffectiveVolumePkcs5, (EffectiveVolumePim < 0)? VolumePim : EffectiveVolumePim, FALSE, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, 0);
	}
	...
	while (mounted == 0)
	{
		...
		//Joakim
		mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &VolumePassword, VolumePkcs5, VolumePim, VolumeTrueCryptMode, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, !Silent, 0);
		NormalCursor ();
		...
		foreach (const HostDevice &drive, devices)
		{
			...
			foreach (const HostDevice &device, partitions)
			{
				...
				if (!mounted)
				{
					...
					//Joakim
					if ((mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, &VolumePassword, VolumePkcs5, VolumePim, VolumeTrueCryptMode, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, TRUE, FALSE, 0)) > 0
						|| ((VolumePassword.Length > 0) && ((mounted = MountVolume (hwndDlg, nDosDriveNo, szFileName, NULL, VolumePkcs5, VolumePim, VolumeTrueCryptMode, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, TRUE, FALSE, 0)) > 0)))
					{
	...
	case WM_INITDIALOG:
		{
			...
			//Joakim
			ContainerOffset = CmdContainerOffset;
					...
					else
					{
						...
						if (CmdVolumePassword.Length != 0 || (FirstCmdKeyFile && (CmdVolumePasswordValid || bEffectiveTryEmptyPasswordWhenKeyfileUsed)))
						{
							...
							mounted = MountVolume (hwndDlg, szDriveLetter[0] - L'A',
								...
								&mountOptions, Silent, reportBadPasswd, ContainerOffset);
						...
						else
						{
							// Cached password
							//Joakim
							mounted = MountVolume (hwndDlg, szDriveLetter[0] - L'A', szFileName, NULL, EffectiveVolumePkcs5, CmdVolumePim, EffectiveVolumeTrueCryptMode, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, Silent, FALSE, ContainerOffset);
						}
						...
						while (!mounted && !Silent)
						{
							...
							//Joakim
							mounted = MountVolume (hwndDlg, szDriveLetter[0] - L'A', szFileName, &VolumePassword, VolumePkcs5, VolumePim, VolumeTrueCryptMode, bCacheInDriver, bIncludePimInCache, bForceMount, &mountOptions, FALSE, TRUE, ContainerOffset);
	...
	if (nNoCommandLineArgs > 0)
	{
		...
		for (i = 0; i < nNoCommandLineArgs; i++)
		{
			...
			argument args[]=
			{
				...
				{ OptionContainerOffset,		L"/offset",			L"/i", FALSE }, //Joakim
			...
			//Joakim
			case OptionContainerOffset:
				GetArgumentValue (lpszCommandLineArgs, &i, nNoCommandLineArgs, TmpContainerOffset, sizeof (TmpContainerOffset));
				CmdContainerOffset = _wcstoi64 (TmpContainerOffset,NULL,10);
				break;
