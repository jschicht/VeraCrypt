		...
		else if (lw == IDC_NEXT)
		{
				...
				|| nCurPageNo == NONSYS_INPLACE_ENC_RESUME_PASSWORD_PAGE)
			{
				...
				else
				{
					...
					//Joakim
					if (MountVolume (hwndDlg, driveNo, szFileName, &volumePassword, hash_algo, volumePim, FALSE, FALSE, FALSE, TRUE, &mountOptions, FALSE, TRUE, 0) < 1)
					{
						NormalCursor();
						return 1;
					}
...
int MountHiddenVolHost (HWND hwndDlg, wchar_t *volumePath, int *driveNo, Password *password, int pkcs5_prf, int pim, BOOL bReadOnly)
{
	...
	*driveNo = GetLastAvailableDrive ();
	...
	//Joakim
	if (MountVolume (hwndDlg, *driveNo, volumePath, password, pkcs5_prf, pim, FALSE, FALSE, FALSE, TRUE, &mountOptions, FALSE, TRUE, 0) < 1)
	{
		*driveNo = -3;
